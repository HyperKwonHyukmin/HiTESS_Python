...
class HookTrolley(hmNastranBDF_Importer, hmNastranBDF_Exporter, CalcFunc):
  ...
  def HookTrolleyRun(self):
    ...
    # '''
    # # HookTrolley-
    # Bar orientation 오류 시, 해석 오류가 발생하여 이를 방지하기 위한 임시대책
    # '''
    # self.BarOrientationModifier()
    #
    # #############################
    # '''
    # # # HookTrolley-
      ...
      if len(s) == 4:  # HookTrolley 권상포인트 개수가 4개 일 때
        if self.shapeDetectorDict[f'SET{i + 1}'] == '4개점 사각형 형태':  # HookTrolley 권상포인트 형태가가 사각형 일 때, 일직선 제외
          ...
          if abs(max_Z - min_Z) > 100 or max_deg > 10:
            ...
            if debugPrint:
              print("Sling belt 네 점이 직사각형에 가깝지 않음")
            continue  # 다음 SET으로 넘어가서 계속 진행
  ...
  def BarOrientationModifier(self):
    def unit_vector(a, b):
      """a, b: [x,y,z]. returns A->B unit vector (list of 3 floats)."""
      vx = b[0] - a[0]
      vy = b[1] - a[1]
      vz = b[2] - a[2]
      norm = math.sqrt(vx * vx + vy * vy + vz * vz)
      if norm == 0:
        raise ValueError("NodeA and NodeB are identical; cannot compute direction.")
      return [round(vx / norm, 1), round(vy / norm, 1), round(vz / norm, 1)]

    def decide_bar_orientation(beam_dir, eps=1e-1):
      """
      beam_dir: unit vector (or any vector; it will be treated by component magnitude)
      Rule:
        - If beam is mostly along X or Y (±X, ±Y) => plan => orientation [0,0,1]
        - If beam is mostly along Z (±Z)         => section => orientation [1,0,0]
      """
      ax, ay, az = abs(beam_dir[0]), abs(beam_dir[1]), abs(beam_dir[2])

      # 빔이 거의 0벡터면 기본값(Plan 취급)로
      if ax < eps and ay < eps and az < eps:
        return [0.0, 0.0, 1.0]

      # Z가 가장 크면 "Z 방향" => section
      if az >= ax and az >= ay:
        return [1.0, 0.0, 0.0]
      else:
        # X 또는 Y가 우세 => plan
    ...
    with open(self.new_bdf, 'r', encoding='utf8') as f:
      ...
      for i, line in enumerate(lines):
        ...
        if line.split()[0] == 'GRID':
          ...
          node_dict[int(line_split[1])] = [
            round(float(line_split[2]), 1),
            round(float(line_split[3]), 1),
            round(float(line_split[4]), 1),
          ]
          new_lines.append(line)
        elif line.split()[0] == 'CBEAM':
          line_split = line.split()
          CBEAM = line_split[0]
          cbeamID = line_split[1]
          cbeamProperty = line_split[2]
          nodeA_ID = int(line_split[3])
          nodeB_ID = int(line_split[4])
          a = node_dict[nodeA_ID]
          b = node_dict[nodeB_ID]
          beam_dir = unit_vector(a, b)
          orient = decide_bar_orientation(beam_dir)
          BDFtext = f'{CBEAM:<8}{cbeamID:>8}{cbeamProperty:>8}{nodeA_ID:>8}{nodeB_ID:>8}{orient[0]:>8}{orient[1]:>8}{orient[2]:>8}\n'
          new_lines.append(BDFtext)
        else:
          new_lines.append(line)
...
def main():
  try:
    ...
    input_bdf = r"C:\Users\HHI\Desktop\temp\1.bdf"  # 예: 'input.bdf'
    # input_bdf = r"C:\Coding\Python\Projects\ModuleUnit\bdf\InfogetBDF\part2\M04_R01_forGU.bdf "  # 예: 'input.bdf'
    output_bdf = r"C:\Users\HHI\Desktop\temp\2.bdf"  # 예: 'input.bdf'
